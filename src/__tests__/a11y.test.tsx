...
// 主要コンポーネントのアクセシビリティ (a11y) 自動検査 (#491)。
//
// axe-core (vitest-axe ラッパ) で各コンポーネントを実描画し、コントラスト・
// ラベル欠落・ロール不整合などの a11y 違反を検出する。デザイン Epic #453 の
// フォーカスリング (#475) / 型・NULL の描き分け (#474) / カラーブラインド配慮
// (#476) で積み上げた a11y 投資を、回帰から守る受け皿。
//
// jsdom はレイアウトを計算しないため、色コントラスト (`color-contrast`) など
// 実レンダリング寸法に依存するルールは安定して評価できない。これらは #306 の
