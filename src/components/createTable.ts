...
function columnLine(
  ...
  // MSSQL の `IDENTITY(1,1)` は型の直後でなければならず (NOT NULL/UNIQUE の
  // 後ろに置くと構文エラー)、MySQL の `AUTO_INCREMENT` / PostgreSQL の
  // `GENERATED BY DEFAULT AS IDENTITY` (どちらも後方の位置で構わない) とは
  // 位置の制約が異なるため、ここだけ先に積む。
  if (col.autoIncrement && driver === "mssql") parts.push("IDENTITY(1,1)");
  ...
  if (col.autoIncrement) {
    ...
    // SQLite の非 PK 自動採番は無効なので無視する。MSSQL は上で積み済み。
  }
