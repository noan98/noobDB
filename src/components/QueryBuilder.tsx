...
import { isAutoGenerated } from "./testDataGen";
...
// INSERT 行の必須カラム (NOT NULL かつ default なし) マーク。危険度は
// `inlineWarningCss` ほど高くないので警告色ではなく控えめなアスタリスクにするが、
// 「入力なしでは失敗する」ことを示すため error トークンで軽く強調する。
const requiredMarkCss: SystemStyleObject = {
  color: "var(--text-error)",
  fontWeight: 700,
  fontSize: "var(--text-md)",
  flexShrink: 0,
...
/** One `ORDER BY` term (SELECT only). A blank `column` is filtered out at
 *  generation time, same convention as `WhereCondition` / `ColumnValuePair`. */
...
export interface QueryBuilderSnapshot {
  ...
  /**
   * `ORDER BY` terms (SELECT only). Optional for backward compatibility with
   * snapshots persisted before this field existed (`tabPersistence.ts`'s
   * `isValidBuilderSnapshot` tolerates its absence the same way it already
   * does for `whereEnabled` / `limitEnabled`) — an older snapshot restores as
   * if no sort had been configured, rather than being rejected outright.
   */
  orderBy?: OrderByItem[];
...
interface Props {
  ...
  /**
   * Inserts the generated SQL into the caller's editor instead of running it
   * (Query Builder as a starting point for manual editing). When provided, a
   * secondary "Insert into editor" button appears in the footer alongside
   * Run / Dry Run.
   */
...
/**
 * A column INSERT genuinely can't skip (改善 2): NOT NULL, no default value,
 * and not DB-populated (`isAutoGenerated`, shared with `testDataGen.ts`'s
 * "omit auto-generated columns" rule). Used to mark INSERT pair rows with a
 * required indicator — informational only, never blocks Run (an omitted
 * required column simply fails at the server, same as today).
 */
export function isRequiredColumn(info: TableColumnInfo | undefined | null): boolean {
  if (!info) return false;
  return !info.nullable && info.default === null && !isAutoGenerated(info);
}
...
/** Renders the `ORDER BY` clause (leading space included, empty string if no
 *  term has a column). Column names are quoted with `quoteIdentFor`; blank
 *  rows are filtered out, same convention as `renderWhereClause`. */
function renderOrderByClause(driver: string, orderBy: OrderByItem[]): string {
  const rendered = orderBy
    .filter((o) => o.column.trim())
    .map((o) => `${quoteIdentFor(driver, o.column)} ${o.direction}`);
...
export function QueryBuilder({ sessionId, driver, defaultDatabase, defaultTable, initialSnapshot, readOnly, onExecute, onPreview, onPersist, onInsertToEditor, onClose }: Props) {
  const t = useT();
  ...
  // ORDER BY (SELECT のみ)。WHERE と異なり必須ではないので既定は 0 行 —
  // 「+ 追加」を押すまでセクションは空のままで済む。
  const [orderBy, setOrderBy] = useState<OrderByItem[]>(initialSnapshot?.orderBy ?? []);
  ...
  const sql = useMemo(
    () => buildSql(driver, tableColumns, kind, database, table, selectColumns, selectAll, whereEnabled, whereConditions, orderBy, limitEnabled, limit, setPairs, insertPairs),
    [driver, tableColumns, kind, database, table, selectColumns, selectAll, whereEnabled, whereConditions, orderBy, limitEnabled, limit, setPairs, insertPairs],
  );
  ...
  }), [kind, database, table, selectAll, selectColumns, whereEnabled, whereConditions, orderBy, limitEnabled, limit, setPairs, insertPairs]);
  ...
  // エディタへの挿入は実行しないので、Run/Dry Run のような「プレースホルダが
  // 残っていないか」の全チェックは課さない — 未完成の SQL 骨格 (WHERE 条件が
  // まだ無い等) をエディタに置いて手で仕上げるのも正当な使い方。唯一ブロックする
  // のは「テーブル未選択」で、これは `<table>` という無意味なプレースホルダしか
  // 生成できず、挿入先として提示する価値がないため。
  const insertToEditorDisabled = !table.trim();
  const handleInsertToEditor = useCallback(() => {
    if (!onInsertToEditor || insertToEditorDisabled) return;
    onPersist?.(captureSnapshot());
    onInsertToEditor(sql);
    onClose();
  }, [sql, onInsertToEditor, insertToEditorDisabled, onClose, onPersist, captureSnapshot]);
  ...
  const updateOrderBy = (idx: number, patch: Partial<OrderByItem>) => {
    setOrderBy((prev) => prev.map((o, i) => (i === idx ? { ...o, ...patch } : o)));
  };
  const addOrderBy = () =>
    setOrderBy((prev) => [...prev, { column: "", direction: "ASC" }]);
  ...
  /**
   * 「全カラムを追加」(改善 2): テーブルの全カラム (自動採番を除く) を
   * insertPairs へ展開する。既に値が入力済みの列はその値を保持し、テーブルの
   * カラムに存在しない自由入力の行 (describeTable に無い列名) も捨てずに
   * 末尾へ残す — ボタンを押すことでユーザ入力が消えることは無い。
   */
  const addAllInsertColumns = () => {
    if (tableColumns.length === 0) return;
    setInsertPairs((prev) => {
      const existingByCol = new Map(prev.filter((p) => p.column).map((p) => [p.column, p.value]));
      const expandable = tableColumns.filter((c) => !isAutoGenerated(c)).map((c) => c.name);
      const expanded: ColumnValuePair[] = expandable.map((name) => ({
        column: name,
        value: existingByCol.get(name) ?? "",
      }));
      const extra = prev.filter((p) => p.column && !expandable.includes(p.column));
      const merged = [...expanded, ...extra];
  ...
  const showOrderBy = kind === "SELECT";
  const showLimit = kind === "SELECT";
  const showSet = kind === "UPDATE";
  const showInsertValues = kind === "INSERT";

  // 改善 4: カラム選択の ComboSelect に PK/NOT NULL/FK バッジとカラム型 detail
  // を添えるための候補一覧。WHERE/SET/INSERT/ORDER BY のカラム選択で共有する。
  const columnComboOptions: ComboSelectOption[] = useMemo(
    () =>
      tableColumns.map((c) => {
        const badges: { label: string; tone?: "accent" | "muted" | "warning" }[] = [];
        if (c.key.toUpperCase() === "PRI") badges.push({ label: "PK", tone: "accent" });
        if (!c.nullable) badges.push({ label: t("qbBadgeNotNull"), tone: "warning" });
        if (c.referenced_table) badges.push({ label: "FK", tone: "muted" });
        return { value: c.name, detail: c.data_type, badges };
      }),
    [tableColumns, t],
  );
  const operatorComboOptions: ComboSelectOption[] = useMemo(
    () => WHERE_OPERATORS.map((op) => ({ value: op })),
    [],
  );
      ...
      <ModalBody>
        <Box display="flex" flexDirection="column" gap="1.5">
          ...
          {showInsertValues && (
            <chakra.section css={sectionCss}>
              <Box css={sectionRowCss}>
                ...
                <Box css={{ display: "flex", gap: "1.5" }}>
                  <Tooltip
                    label={tableColumns.length === 0 ? t("qbPickTableFirst") : undefined}
                    focusableWrapper={tableColumns.length === 0}
                  >
                    <chakra.button
                      type="button"
                      css={smallBtnCss}
                      onClick={addAllInsertColumns}
              ...
              {insertPairs.map((p, i) => {
                const info = resolveColumnInfo(tableColumns, p.column);
                const notNullWarning = isNullValueOnNotNullColumn(p.value, info);
                const required = isRequiredColumn(info);
                return (
                  <Box css={stackedRowCss} key={`ins-${i}`}>
                    <Box css={rowCss}>
                      ...
                      />
                      {required && (
                        <Tooltip label={t("qbRequiredColumn")}>
                          <chakra.span css={requiredMarkCss} aria-label={t("qbRequiredColumn")}>
                            *
                          </chakra.span>
                        </Tooltip>
                      )}
                      ...
                      <ValueControl
                        ...
                        columnInfo={info}
                        placeholder={info?.default != null ? info.default : t("qbValue")}
                        t={t}
                      />
          ...
          {showOrderBy && (
            <chakra.section css={sectionCss}>
              <Box css={sectionRowCss}>
                <Box css={sectionTitleCss}>{t("qbOrderBy")}</Box>
                <chakra.button type="button" css={smallBtnCss} onClick={addOrderBy}>
                  + {t("qbAddOrderBy")}
                </chakra.button>
              </Box>
              {orderBy.length === 0 ? (
                ...
                orderBy.map((o, i) => (
                  <Box css={rowCss} key={`ob-${i}`}>
                    ...
                    <Select
                      ...
                      value={o.direction}
                      onChange={(e) =>
                        updateOrderBy(i, { direction: e.target.value as "ASC" | "DESC" })
                      }
                    >
