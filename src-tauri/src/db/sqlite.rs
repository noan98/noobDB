...
impl SqliteConn {
    ...
    pub async fn preview_execute_with_limit(
        ...
        if super::has_stacked_statements_for(super::DriverKind::Sqlite, sql) {
            return Err(AppError::InvalidInput(
...
/// `DriverKind` を渡す (#1051 — SQLite は `\` をただの文字として扱う)。
...
pub(crate) fn is_query_shape(sql: &str) -> bool {
    ...
    if trimmed.starts_with("with") {
        return !super::mysql::with_cte_is_mutation(super::DriverKind::Sqlite, sql);
    }
...
fn strip_sql_comments(sql: &str) -> String {
    super::strip_sql_comments(sql, super::SqlFlavor::Sqlite)
}
