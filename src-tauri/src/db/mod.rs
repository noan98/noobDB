...
/// [`classify_write_kind_for`]) used to mask with the MySQL-flavoured
/// MySQL-flavoured mask regardless of the target driver, which **fails open**
/// on the other dialects: given `SELECT '\'; DELETE FROM t; --'`, the MySQL
/// reading treats `\'` as an escaped quote and swallows the `; DELETE …` as
...
mod tests {
    ...
    fn read_only_rejects_mssql_locking_table_hints() {
        for sql in [
            ...
            assert!(
                !is_read_only_sql_for(DriverKind::Mssql, sql),
                "expected {sql:?} to be rejected as a locking read"
            );
            assert!(
                !is_read_only_sql(sql),
                "driver-less must reject {sql:?} too"
            );
        }
    }
    ...
    fn read_only_allows_non_locking_hints_and_lookalikes() {
        for sql in [
            ...
            assert!(
                is_read_only_sql_for(DriverKind::Mssql, sql),
                "expected {sql:?} to stay read-only"
            );
            assert!(is_read_only_sql(sql), "driver-less must accept {sql:?} too");
        }
    }
