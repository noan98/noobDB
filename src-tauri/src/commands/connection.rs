...
use crate::db::aws_iam::{self, AwsIamConfig};
...
pub struct ConnectRequest {
    ...
    /// AWS RDS / Aurora IAM database authentication (#734). When set, `password`
    /// is ignored and a fresh RDS auth token is generated per physical
    /// connection; TLS is forced to `require` or stricter. MySQL / PostgreSQL only.
    #[serde(default)]
...
fn log_attempt(req: &ConnectRequest, op: &str) {
    tracing::info!(
        ...
        db_auth = if req.aws_iam.is_some() { "aws_iam" } else { "password" },
...
async fn build_options(
    ...
    // AWS IAM auth (#734): resolve the signing parameters *before* the SSH
    // tunnel swaps `host` for 127.0.0.1 — the token must be signed for the real
    // RDS endpoint name. The token itself is generated by the driver per
    // physical connection (never stored on the session), and TLS is forced.
    let (aws_iam, password, ssl_mode) = match &req.aws_iam {
        Some(cfg) => {
            let iam = iam_options_for(req.driver, cfg, &req.host, req.port)?;
            (
                Some(iam),
                String::new(),
                Some(aws_iam::enforce_tls(req.ssl_mode)),
            )
        }
        None => (None, resolve_password(req)?, req.ssl_mode),
    };
...
/// Validate an AWS IAM auth request and resolve its signing parameters (#734).
/// RDS IAM database authentication exists only for MySQL / PostgreSQL engines.
fn iam_options_for(
    driver: DriverKind,
    cfg: &AwsIamConfig,
    host: &str,
    port: u16,
) -> Result<aws_iam::AwsIamOptions> {
    if !matches!(driver, DriverKind::Mysql | DriverKind::Postgres) {
        return Err(AppError::InvalidInput(format!(
            "AWS IAM authentication is only supported for MySQL / PostgreSQL (got {})",
            driver.as_str()
        )));
    }
    aws_iam::build_options(cfg, host, port, &aws_iam::process_env)
}
...
mod tests {
    ...
    #[test]
    fn iam_options_are_signed_for_the_pre_tunnel_endpoint_and_mysql_pg_only() {
        let cfg = AwsIamConfig {
            region: "us-east-1".into(),
            profile: Some("work".into()),
        };
        // The endpoint captured for signing is the request's own host/port —
        // i.e. what the user typed, before any SSH tunnel substitution.
        let o = iam_options_for(DriverKind::Postgres, &cfg, "db.abc.rds.amazonaws.com", 5432)
            .expect("postgres ok");
        assert_eq!(o.endpoint_host, "db.abc.rds.amazonaws.com");
