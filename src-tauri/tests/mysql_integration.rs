...
#[tokio::test]
async fn mysql_upsert_import_roundtrip() {
    let Ok(url) = std::env::var("NOOBDB_TEST_MYSQL_URL") else {
        eprintln!("skip: NOOBDB_TEST_MYSQL_URL not set");
        return;
    };
    let opts = t::parse_mysql_url(&url).expect("valid url");
    let conn = t::connect(&opts).await.expect("connect");
    conn.execute("DROP TABLE IF EXISTS noobdb_my_upsert", None)
        .await
        .expect("drop");
    conn.execute("CREATE TABLE noobdb_my_upsert (id INT PRIMARY KEY, name VARCHAR(50) NOT NULL) ENGINE=InnoDB", None).await.expect("create");
    conn.execute(
        "INSERT INTO noobdb_my_upsert VALUES (1, 'a'), (2, 'b')",
        None,
    )
    .await
    .expect("seed");
    assert_upsert_roundtrip(&conn, "noobdb_my_upsert").await;
    conn.execute("DROP TABLE noobdb_my_upsert", None)
        .await
        .expect("cleanup");
}
