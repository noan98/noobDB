...
#[tokio::test]
async fn sqlite_upsert_import_roundtrip() {
    let mut p = std::env::temp_dir();
    p.push(format!("noobdb_sqlite_upsert_{}.db", std::process::id()));
    let _ = std::fs::remove_file(&p);
    std::fs::File::create(&p).expect("create temp sqlite file");
    let opts = t::sqlite_options(p.to_str().expect("utf8 path"));
    let conn = t::connect(&opts).await.expect("connect");
    conn.execute(
        "CREATE TABLE imp_up (id INTEGER PRIMARY KEY, name TEXT NOT NULL)",
        None,
    )
    .await
    .expect("create");
