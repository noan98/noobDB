...
- アプリ内アクティビティ / 通知センター — `activityLog.ts` + `components/ActivityCenter.tsx`
  ...
  一覧の行とフィルタチップは `SeverityLog.tsx` に切り出してあり、Bottom Panel の
  「アクティビティ」「メッセージ」タブ (#1114) と共有する (ポップオーバーは相対時刻、
  パネルは開きっぱなしで古くなるため絶対時刻)。
- 機微カラムの表示マスキング (#1069) — `columnMask.ts` (純ロジック) + `ResultGrid` の
  `DataGrid` 結線。画面共有/デモ時に `email` / `token` / `ssn` などの列をグリッド上で
  固定長の伏せ字 (`••••`) にする**表示専用**機能で、実値・ソート・編集バッファ・
  エクスポート内容は不変 (エクスポートのマスキングは #733 の担当)。対象は「設定
  `columnMaskEnabled` がオン」かつ「列ヘッダメニューの列単位の上書き (`gridViewState`
  の `masks`、列名キー) → 無ければ列名パターン `columnMaskPatterns` (部分一致・
  大文字小文字無視、`*`/`?` を含むものは全体グロブ)」で決まる (`resolveMaskedColumns`)。
  ...
  TSV 選択) は設定 `columnMaskCopyPlaceholder` (既定オン) で伏せ字、SQL としてコピーは
  (伏せ字入りの壊れた SQL になるため) 同設定の間は無効、行の複製・値ビューア・FK
  ジャンプ・クイックフィルタ・列の統計は無効/非表示、集計フッターと選択サマリは値を
  出さない、行インスペクタは伏せ字 + フィールドコピー無効、インライン編集の開始
  (ダブルクリック/Enter/打鍵) は reveal するまでブロック (値を書くだけのクイック
  セット・Delete で NULL・貼り付けはそのまま使える)、保留中の編集値も伏せ字で表示。
  結果内検索 (#644) のヒット強調と全文フィルタは実値で判定する (入力者自身が知って
